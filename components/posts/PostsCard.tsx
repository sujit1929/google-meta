            ...
            <div className="flex items-center justify-between pt-4 border-t border-slate-100">
              <div className="flex items-center gap-6">
                ...
                <button onClick={handleLike} className="group flex items-center gap-2 hover:scale-105 transition">
                  ...
                  <span className={`font-medium ${isLiked ? "text-red-500" : "text-slate-600"}`}>{post.likes.length}</span>
                </button>
                  ...
                  <div key={comment._id} className="flex items-start gap-3 p-4 bg-slate-50 rounded-xl border hover:bg-slate-100 transition">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <p className="font-semibold text-slate-900 text-sm">{comment.user.username}</p>
                        <p className="text-xs text-slate-500">{typeof comment.timestamp === "string" ? comment.timestamp : new Date(comment.timestamp).toLocaleString()}</p>
                      </div>
